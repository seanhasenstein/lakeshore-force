...
TeamSchema.statics.addAthlete = function(id, args) {
  ...
  return this.findById(id).then(team => {
    const athlete = new Athlete(args);
    team.athletes.push(athlete);
    return Promise.all([athlete.save(), team.save()]).then(([athlete, team]) => team);
  });
};
...
TeamSchema.statics.addCoach = function(id, args) {
  ...
  return this.findById(id).then(team => {
    ...
    return Promise.all([coach.save(), team.save()]).then(([coach, team]) => team);
  });
};
